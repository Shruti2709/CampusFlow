...
import { Eye, EyeOff, GraduationCap, Mail, Lock, ArrowRight, Sparkles, Home } from "lucide-react";
...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || "Invalid credentials. Please verify your email and password.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#fcfbf9] relative flex items-center justify-center px-4 py-12 overflow-hidden">
      ...
      <div className="pointer-events-none absolute -top-32 -left-32 w-96 h-96 rounded-full bg-blue-100/60 blur-[90px]" />
      <div className="pointer-events-none absolute -bottom-32 -right-32 w-96 h-96 rounded-full bg-purple-100/60 blur-[90px]" />
      ...
      <motion.div
        ...
        className="relative z-10 w-full max-w-md rounded-3xl border border-stone-200/90 bg-white p-8 sm:p-10 shadow-xl shadow-stone-900/5"
      >
        {/* Header */}
        <div className="text-center mb-7">
          ...
          <p className="text-xs sm:text-sm text-stone-500 mt-1">
            Sign in to access your placement &amp; campus hub
          </p>
        </div>
        ...
        <div className="mt-6 pt-5 border-t border-stone-100">
          ...
          <div className="grid grid-cols-3 gap-2">
            <button
              type="button"
              onClick={() => fillDemo("student@campus.edu", "password123")}
              className="py-2 px-2 rounded-xl text-xs font-semibold bg-indigo-50 text-indigo-700 border border-indigo-100 hover:bg-indigo-100 transition truncate"
            >
              🎓 Student
            </button>
            <button
              type="button"
              onClick={() => fillDemo("recruiter@google.com", "password123")}
              className="py-2 px-2 rounded-xl text-xs font-semibold bg-purple-50 text-purple-700 border border-purple-100 hover:bg-purple-100 transition truncate"
            >
              💼 Recruiter
            </button>
            <button
              ...
              className="py-2 px-2 rounded-xl text-xs font-semibold bg-emerald-50 text-emerald-700 border border-emerald-100 hover:bg-emerald-100 transition truncate"
            >
              🛡️ Admin
            </button>
          </div>
